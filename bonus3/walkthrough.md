...
`fopen` is call with this string `/home/user/end/.pass`. `fread read 2 times. And `execl` has for parameters `/bin/sh` and `sh`.

if we want to exec sh, we need to the string result to frend be equal to av[1]
```
   0x080485c7 <+211>:	mov    eax,DWORD PTR [ebp+0xc]
   0x080485ca <+214>:	add    eax,0x4
   0x080485cd <+217>:	mov    eax,DWORD PTR [eax]
...
Luckly, we can modify this string thank this instructions :
```
   0x08048584 <+144>:	call   0x8048430 <atoi@plt>
   0x08048589 <+149>:	mov    BYTE PTR [esp+eax*1+0x18],0x0
```

`atoi` return 0 for a null string. so :
