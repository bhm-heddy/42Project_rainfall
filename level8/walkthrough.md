...
Symbol table '.dynsym' contains 14 entries:
   Num:    Value  Size Type    Bind   Vis      Ndx Name
     ...
     8: 00000000     0 FUNC    GLOBAL DEFAULT  UND system@GLIBC_2.0 (2)
     9: 00000000     0 NOTYPE  WEAK   DEFAULT  UND __gmon_start__
    10: 00000000     0 FUNC    GLOBAL DEFAULT  UND __libc_start_main@GLIBC_2.0 (2)
    11: 08049aa0     4 OBJECT  GLOBAL DEFAULT   25 stdout@GLIBC_2.0 (2)
    12: 0804880c     4 OBJECT  GLOBAL DEFAULT   15 _IO_stdin_used
    13: 08049a80     4 OBJECT  GLOBAL DEFAULT   25 stdin@GLIBC_2.0 (2)
```

Have 2 global variables in .bss:

```
gdb-peda$ x 0x8049ab0
   0x8049ab0 <service>
gdb-peda$ x 0x8049aac
   0x8049aac <auth>
```

And we have 4 strings for 4 distinct parts in the main function. Each part, test if the fgets result is equal to **"auth "** or **service** or **login** or **reset**. In **login** part there is a *system* call with */bin/sh* argument. For run this call we need to `auth+32` is not null.

Serval solutions, once is :
So we have to trigger first malloc with `auth ` command and increase heap memory (align to 16) 2 times with `service` command
